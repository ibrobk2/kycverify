...
// Function to populate LGA dropdown for a selected state
function populateLGADropdown(stateSelect, lgaSelect, loaderElement) {
    const selectedState = stateSelect.value;
    lgaSelect.innerHTML = '<option value="">Select LGA</option>';

    if (selectedState && nigeriaData.lgas[selectedState]) {
        // Show loader
        if (loaderElement) {
        ...
        setTimeout(() => {
            nigeriaData.lgas[selectedState].forEach(lga => {
                const option = document.createElement('option');
                option.value = lga;
        ...
        // Hide loader if no state selected
        if (loaderElement) {
            loaderElement.style.display = 'none';
        }
    }
